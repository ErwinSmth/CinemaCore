...
*   **`POST /movies/tmdb/import/{tmdbId}`**
    ...
    *   **Propósito:** Descarga toda la información de TMDB, la comprime en `JSONB` y crea el registro en PostgreSQL con estado `INACTIVO`.
...
*   **`GET /movies/{id}`**
    ...
    *   **Propósito:** Retorna el documento completo de una película específica leyendo el `JSONB` (sinopsis, actores, trailers).
    *   **Nota de DTO (Aplanamiento):** El API Spec define que la respuesta no expone un mapa genérico de `metadata`. En su lugar, el backend extrae del `JsonNode` de Hibernate propiedades explícitas y tipadas (ej. `posterPath`, `backdropPath`, `trailerUrl`) y las envía en el primer nivel del JSON de respuesta. Esto previene fallos de serialización en el proxy.
